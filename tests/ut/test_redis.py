...
import asyncio
import hashlib
from unittest.mock import call, patch, AsyncMock, AsyncMockMixin, Mock
...
class TestInstance:
    ...
    @pytest.mark.parametrize("connection, expected_address, expected_kwargs", [
        (
            ('localhost', 6379),
            None,
            {
        ...
        ("redis://host:6379/0?encoding=utf-8", "redis://host:6379/0?encoding=utf-8", {})
    ])
    @pytest.mark.asyncio
    async def test_connect_pool_not_created(self, connection, expected_address, expected_kwargs, fake_client):
        with patch(
            ...
            create_redis_pool.assert_called_once_with(expected_address, **expected_kwargs, max_connections=100)
    ...
    async def test_connect_pool_not_created_with_max_connections(self, fake_client):
        connection = {'host': 'localhost', 'port': 6379, 'db': 0, 'password': 'pass', 'max_connections': 5}
        with patch(
                'aioredlock.redis.Instance._create_redis',
                AsyncMock(return_value=fake_client)
        ) as create_redis_pool:
    ...
    async def test_connect_pool_aioredis_instance(self, mocker, fake_client):
        ...
        mocker.patch('redis.asyncio.Redis.from_pool', return_value=fake_client)
    ...
    @pytest.mark.asyncio
    async def test_lock_sleep(self, fake_instance):
    ...
    async def test_lock_without_scripts(self, fake_instance, func, args, expected_keys, expected_args):
        ...
        await instance.connect()
        redis_client = instance._client
        redis_client.evalsha.side_effect = [ResponseError('NOSCRIPT'), AsyncMock(return_value=True)]
        ...
        redis_client.evalsha.assert_called_with(
            getattr(instance, '{0}_script_sha1'.format(func)), 1, *expected_keys, *expected_args
        )
...
class TestRedis:
    ...
    @pytest.mark.asyncio
    @parametrize_methods
    async def test_lock(self, mock_redis_two_instances, fake_client, method_name, call_args):
    ...
    async def test_is_locked(self, mock_redis_two_instances, fake_client, get_return_value, locked):
        redis = mock_redis_two_instances
    ...
    async def test_lock_one_of_two_instances_failed(
            self, mock_redis_two_instances, fake_client,
